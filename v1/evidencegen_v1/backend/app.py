...
from templates_config import CONTROL_TEMPLATES, SOURCE_LABELS, get_template, build_custom_template
from agents import run_agents
from llm import llm_client
from llm.mapping import map_evidence_to_requirements, summarize_pack
from llm.qa import answer_question
from llm.audit_intake import analyze as analyze_audit_text
import storage
...
def api_audit_intake():
    body = request.get_json(force=True) or {}
    audit_text = body.get("audit_text", "")
    result, engine = analyze_audit_text(audit_text)
...
def api_create_pack():
    ...
    if template_id == "custom-audit-intake" or custom_requirements:
        template = build_custom_template(
            custom_requirements or [],
            name=body.get("custom_template_name", "Custom — Audit Intake"),
        )
    else:
    ...
    pack = {
        ...
        "source_audit_text": body.get("audit_text"),
