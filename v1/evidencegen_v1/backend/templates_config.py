...
def build_custom_template(requirement_texts, name="Custom — Audit Intake"):
    """Builds an ad-hoc template from free-text requirement lines, for the
    Audit Intake -> Evidence Collection flow (no fixed template needed)."""
    requirements = []
    ...
    if not requirements:
        requirements = [{
            "id": "custom-req-1",
            "text": "Evidence supports the audit request",
            "checks_for": "General evidence relevant to the audit intake text.",
        }]
    return {
        "id": "custom-audit-intake",
        "name": name,
        "description": "Ad-hoc template generated from an Audit Intake request.",
        "default_sources": ["repo", "docs", "screenshots"],
